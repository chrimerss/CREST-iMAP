/*
 * wq.app v1.1.2 - @wq/app 1.1.2
 * Utilizes @wq/store and @wq/pages to dynamically load and render content from a wq.db-compatible REST service
 * (c) 2012-2019, S. Andrew Sheppard
 * https://wq.io/license
 */

define(['jquery.mobile', './store', './model', './outbox', './template', './router', './spinner'], function (jQM, ds, modelModule, outbox, tmpl, router, spin) { 'use strict';
...
spin = spin && spin.hasOwnProperty('default') ? spin['default'] : spin;
...
app.init = function (config) {
    ...
    app.spin = spin; // Router (wq/router.js) configuration
    ...
    app.can_login = !!config.pages.login; // Initialize wq/store.js

    ds.init(config.store);
    app.store = ds;
    app.service = ds.service; // Initialize wq/outbox.js

    outbox.init(config.outbox); // Initialize wq/router.js

    router.init(config.router);
    app.base_url = router.info.base_url; // Initialize wq/template.js
    ...
    tmpl.setDefault('svc', app.service); // Option to submit forms in the background rather than wait for each post
    ...
    if (app.can_login) {
        router.register('logout/?', app.logout); // Load some values from store - not ready till this is done.
    ...
    router.addRoute('outbox/<slug>/edit', 's', _showOutboxItem('edit')); // Fallback index page

    if (!root && !app.wq_config.pages.index && config.template.templates.index) {
        router.register('', function (match, ui) {
            var context = {};
            context.pages = Object.keys(app.wq_config.pages).map(function (page) {
...
app.go = function (page, ui, params, itemid, mode, url, context) {
    ...
    if (itemid) {
        if (itemid == 'new') {
            return _displayItem(itemid, {}, page, ui, params, mode, url, context);
        } else {
            var localOnly = !app.config.loadMissingAsJson;
            return model.find(itemid, 'id', localOnly).then(function (item) {
                _displayItem(itemid, item, page, ui, params, mode, url, context);
            });
        }
    } else {
...
app.runPlugins = function (page, mode, itemid, url, parentInfo) {
    var lastRoute = router.info,
        ...
        routeInfo,
        getItem;
    routeInfo = _getRouteInfo(page, mode, itemid, url.replace(app.base_url + '/', ''), parentInfo);

    if (itemid) {
        if (lastRoute.path == routeInfo.path && context && (context.id || 'new') == itemid) {
...
function _getRouteInfo(page, mode, itemid, url, parentInfo) {
    ...
    router.setPath(url);
    return _objectSpread({}, parentInfo, router.info, {
        page: page,
        page_config: conf,
        mode: mode,
        item_id: itemid
    ...
    function goUrl(ppage, url) {
        return function (match, ui, params) {
            ...
            app.models[ppage].find(match[1]).then(function (pitem) {
                ...
                var context = {
                    parent_id: match[1],
                    parent_url: pitem && pconf.url + '/' + pitem.id || 'outbox/' + match[1].split('-')[1],
...
function _displayList(page, ui, params, url, context) {
    ...
    return getUnsynced().then(function (unsynced1) {
        ...
        return _addLookups(page, context, false, routeInfo).then(function (context) {
            return router.go(url, page + '_list', context, ui, conf.once ? true : false);
        });
    });
...
function _renderOther(page, ui, params, url, context) {
    ...
    Promise.all(_callPlugins('context', undefined, [context, routeInfo])).then(function (pluginContext) {
        pluginContext.forEach(function (pc) {
            context = _objectSpread({}, context, pc);
        });
...
function _renderOutboxItem(mode) {
    ...
    return function (match, ui, params) {
        outbox.loadItem(match[1]).then(function (item) {
            var id,
                idMatch = item.options.url.match(new RegExp(item.options.modelConf.url + '/([^/]+)$'));
            ...
            if (id == 'new') {
                context = _objectSpread({}, context, item.data);
            } else {
                context.id = id;
            }

            _displayItem(id, item.data, item.options.modelConf.name, ui, params, mode, url, context).then(function ($page) {
                if (mode == 'edit' && item.error) {
...
function _showOutboxItem(mode) {
    return function (match) {
        ...
        app.runPlugins(context.page_config.name, mode, context.id || 'new', match[0]);
    };
    ...
    if (modelConf.list && item.synced) {
        ...
        if (item.deletedId) {
            ...
            return model.update([result]).then(function () {
                return Promise.all(_callPlugins('onsave', undefined, [item, result]));
            });
        }
...
function _saveLogin(result) {
    ...
    tmpl.setDefault('user', user);
    tmpl.setDefault('is_authenticated', true);
    return Promise.all([ds.set('/config', config), ds.set('user', user), _setCSRFToken(csrftoken)]).then(function () {
        $('body').trigger('login');
    });
}
...
function _addLookups(page, context, editable, routeInfo) {
    ...
    return Promise.all(queue).then(function (results) {
        ...
        return Promise.all(_callPlugins('context', undefined, [context, routeInfo]));
...
function _this_parent_lookup(field) {
    var model = app.models[field['wq:ForeignKey']];
    return Promise.all([_getOutboxRecordLookup(model), model.getIndex('id')]).then(function (results) {
        return function () {
