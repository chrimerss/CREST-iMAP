...
def regression(input_file, output_file, column = 0, delim = ' ', cv = True, fold = 10, plot = True):
    ...
    model = linear_model.SGDRegressor(alpha = 0.1)
    ...
    if cv:
        ...
        if plot:
            ...
            plt.xlabel('Stochastic Gradient Descent Regression')
            plt.ylabel('Mean Square Error')
            plt.title('Cross validation results of Stochastic Gradient Descent regression model')
            plt.show()
