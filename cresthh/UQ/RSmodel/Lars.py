...
def regression(input_file, output_file, weights = 'uniform', column = 0, delim = ' ', cv = True, fold = 10, plot = True):
    ...
    if cv:
        ...
        if plot:
            ...
            plt.xlabel('Lasso path using LARS Regression')
            plt.ylabel('Mean Square Error')
            plt.title('Cross validation results of LassoLARS regression model')
            plt.show()
