...
def optimization(lb, ub, pf, ieqcons=[], f_ieqcons=None, args=(), kwargs={}, 
        swarmsize=100, omega=0.5, phip=0.5, phig=0.5, maxiter=100, 
        minstep=1e-8, minfunc=1e-8, debug=True, plot=True):
    """
    ...
    assert hasattr(functn.evaluate, '__call__'), 'Invalid function handle'
    lb = np.array(lb)
    ub = np.array(ub)
    ...
    obj = lambda x: functn.evaluate(x, *args, **kwargs)
    ...
    # Calculate the objective value at the current particle's
    fp = obj(p)
