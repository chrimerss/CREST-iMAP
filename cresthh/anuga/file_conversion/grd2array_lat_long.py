...
def grd2array_lat_long(filename, verbose=False, proj=None):
        ...
        #convert to UTM zone
        xurcorner= xllcorner+cellsize*(ncols-1)
        yurcorner= yllcorner+cellsize*(nrows-1)
        print xllcorner, xurcorner
        myproj= Proj(proj)
        xllcrn, yllcrn= myproj(xllcorner, yllcorner)
        xurcrn, yurcrn= myproj(xurcorner, yurcorner)
        ...
        x = num.linspace(xllcrn, xurcrn, ncols)
        y = num.linspace(yllcrn, yurcrn, nrows)
